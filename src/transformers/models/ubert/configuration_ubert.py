...
class UbertConfig(PretrainedConfig):
    ...
    instantiate a UBERT model according to the specified arguments, defining the model architecture. Instantiating a
    configuration with the defaults will yield a similar configuration to that of the UBERT
    ...
    Args:
        vocab_size (`int`, *optional*, defaults to 30522):
            Vocabulary size of the UBERT model. Defines the number of different tokens that can be represented by the
            `inputs_ids` passed when calling [`UbertModel`] or [`TFUbertModel`].
        ...
        type_vocab_size (`int`, *optional*, defaults to 2):
            The vocabulary size of the `token_type_ids` passed when calling [`UbertModel`] or [`TFUbertModel`].
    ...
    ```python
    >>> from transformers import UbertModel, UbertConfig

    >>> # Initializing a UBERT  style configuration
    >>> configuration = UbertConfig()

    >>> # Initializing a model from the  style configuration
    >>> model = UbertModel(configuration)
